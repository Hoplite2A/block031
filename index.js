...
app.get("/api/v1/pets/owner-query", (req, res) => {
  ...
  //TODOD----   const owner = myString.toLowerCase(req.query.owner);

  // find the pet in the pets array
  const pet = pets.find((pet) => pet.owner === owner);
  //TODOD----   const pet = pets.find((pet) => myString.toLowerCase(pet.owner) === owner);
...
app.get("/api/v1/pets/:name", (req, res) => {
  ...
  //TODOD----   const name = myString.toLowerCase(req.params.name);
  console.log(name);

  // find the pet in the pets array
  const pet = pets.find((pet) => pet.name === name);
  //TODOD----   const pet = pets.find((pet) => myString.toLowerCase(pet.name) === name);
